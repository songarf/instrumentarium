...
**Статус:** Рабочий MVP. CI/CD для трех платформ (Windows/Linux/macOS). Активная итерация: macOS поддержка, оптимизация загрузки, улучшение UX.
...
> **2026-06-01:** Оптимизация Windows-сборки: удалён CEF (cefpython3) из	spec файла — билд на ~100+ MB меньше, быстрая распаковка. Убраны sleep(0.5) из shutdown handlers — мгновенное закрытие. CI артефакты переименованы в linux/windows/macos-instrumentarium. Коммит `14e3d7c`. Исправлен git remote для `/opt/data` — переключен на SSH (`git@github.com:songarf/hermes-data.git`), крон-задача `daily_memory_maintenance` обновлена. Коммит `cabf2d5`.
